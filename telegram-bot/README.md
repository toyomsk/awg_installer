...
Все команды с параметрами работают интерактивно: после команды бот запрашивает ввод. Выход из режима: `/cancel` или текст «отмена» / «cancel».

- `/start` или `/help` — Справка по командам
- `/add_client` — Создать клиента (далее ввод имени)
- `/list_clients` — Показать всех клиентов
- `/get_config` — Получить конфиг клиента (далее ID или имя)
- `/delete_client` — Удалить клиента (далее ID из списка, затем подтверждение)
- `/status` — Статус сервера и VPN
- `/restart` — Перезапустить VPN сервер
- `/cancel` — Выход из интерактивного режима ввода

## Использование

1. Найдите бота в Telegram по username
2. Отправьте `/start`
3. Создайте клиента: `/add_client`, затем введите имя (например, `phone`)
4. Получите конфиг: `/get_config`, затем введите ID или имя
...
│   ├── awg_manager.py       # Управление клиентами Amnezia WG
│   ├── db.py                # SQLite: клиенты (id, name), источник истины
│   ├── xray_manager.py      # Управление клиентами Xray (VLESS Reality)
